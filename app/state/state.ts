...
import {
  ...
  SingleCar,
  SinglePeriod,
} from "../CORS/entities/apiExchange/serverTypes";
import {
  getCarList,
  getPlaceList,
  getCarPeriodList,
} from "../CORS/querySender";
import { SingleCarWithPeriods } from "../entities/carPeriods";
import {
  formatCarModelFromBaseToSelect,
  formatCarModelFromHashToSelect,
  dateForServer,
  currentYearForServer,
  nextYearForServer,
  splitDateByMinutes,
  validateField,
  formatCarModelFromSelectToHash,
...
import {
  correctionSecondTimeAfterFirst,
  timeSelectorBy15Min,
...
function trimMultiplePeriodsBy3HoursOnEachSide(
...
function reformatDate(period: Date): string {
  return period.toString().replace(" ", "T");
}
...
function isWithinIntervalsAndFindIt(
  periods: SinglePeriod[],
  timestamps: Date[]
  ...
  const periodsMoreThen2Days = periods.filter((period) => {
    return (
      new Date(period.end).valueOf() - new Date(period.begin).valueOf() >
      1000 * 60 * 60 * 24 * 2
    );
  });
  ...
  periodsMoreThen2Days.forEach(
    (period) =>
      isAfter(timestamps[0], new Date(period.begin)) &&
      isBefore(timestamps[timestamps.length - 1], new Date(period.end))
  );
...
export class State {
  ...
  public setFirstDateOfRange(timestampOfFirstSelectDate: Date): void {
    const arrayForGenerateHTML = this.getFreeTimeSlotsForReceiveAndReturnCar(
      timestampOfFirstSelectDate
    );
    this.firstDateOfRange = timestampOfFirstSelectDate;
    timeSelectorBy15Min(
      "receive",
      shared.domElementId.selectReceiveTimeId,
      arrayForGenerateHTML
    );

    $("#" + `${shared.domElementId.selectReceiveTimeId}`).attr(
      "disabled",
      null
    );

    $("#" + `${shared.domElementId.receiveDataId}`).val(
      this.getFirstDateOfRange().toLocaleDateString()
    );

    validateField(
      shared.domElementId.receiveDataId,
      shared.domElementId.receiveDateTextId
    );
  }
  ...
  public setFirstTimeOfRange(ftr: Date | undefined): void {
    this.firstTimeOfRange = ftr;
  }
  public getFirstTimeOfRange(): Date | undefined {
    const ftr = this.firstTimeOfRange;
    return ftr;
  }

  private secondTimeOfRange: Date | undefined = undefined;
  public setSecondTimeOfRange(ftr: Date | undefined): void {
    this.secondTimeOfRange = ftr;
  }
  public getSecondTimeOfRange(): Date | undefined {
    const ftr = this.secondTimeOfRange;
    return ftr;
  }
  ...
  public setSecondDateOfRange(timestampOfSecondSelectDate: Date): void {
    ...
    const selectedTime = $(`#${shared.domElementId.selectReceiveTimeId}`)
      .val()
      ?.toString()
      .split(":")
    ...
    if (selectedTime && selectedTime[0]) dt?.setHours(selectedTime[0]);

    if (selectedTime && selectedTime[1]) dt?.setMinutes(selectedTime[1]);
    if (dt) this.filterCurrentCarForBookingBySelection(dt);

    $(`#${shared.domElementId.selectReturnTimeId}`).attr(
      "disabled",
      "disabled"
    );
  ...
  public setMainCar() {
    this.mainCarForBid =
      this.freePeriodsForCurrentBookingCarAfterFirstSelect[0].car_id;
  }
  /**
   * @description адреса места для выдачи и возврата арендованных авто
   */
  private placesForReceiveAndReturnCars: PlacesResponse = {
    result_code: 0,
    places: [],
  };
  /**
  ...
  public filterCurrentCarForBookingBySelection(timestamp: Date): void {
    const splitDate: Date[] = eachMinuteOfInterval(
      {
        start: timestamp,
        end: new Date(
    ...
    this.freePeriodsForCurrentBookingCarAfterFirstSelect =
      this.freePeriodsForCurrentBookingCar.filter((carsWithPeriods) =>
        isWithinIntervals(carsWithPeriods.car_periods, splitDate)
      );
  }
  /**
  ...
  private async fetchFreePeriodsForAllCars(): Promise<void> {
    ...
    this.allCarsForRent.cars.forEach((car) => {
    ...
    const resultOfFetchFreePeriods = await Promise.all(
      promisesForFetctFreePeriodsDate
    );

    resultOfFetchFreePeriods.forEach((res, inx) => {
      res.car_periods = reformatDateForIOS(res.car_periods);

      this.freePeriodsForAllBookingCar.push({
        ...this.allCarsForRent.cars[inx],
        car_periods: trimMultiplePeriodsBy3HoursOnEachSide(res.car_periods),
      });
    });

    this.freePeriodsForCurrentBookingCar =
      this.freePeriodsForAllBookingCar.filter(
        (carPeriodItem: SingleCarWithPeriods) => {
          return this.allCarsForCurrentBooking.find(
            (item: SingleCar, inx: number) => {
  ...
  public async init(): Promise<State> {
    ...
    places.places = places.places.filter((place) => !place.archive);
  ...
  private findFirstPeriodWhichConsistTimestamt(
    periods: SinglePeriod[],
    timestamp: Date
  ): boolean {
    const findedPeriod: SinglePeriod | undefined = periods.find((item) => {
  ...
  public isDateBusy(dt: Date): Boolean {
    ...
    if (this.isFirstDateOfRangeWasSelect()) {
      ...
      for (
        let i = 0;
        i < this.freePeriodsForCurrentBookingCarAfterFirstSelect.length;
        ++i
      ) {
        const periods =
          this.freePeriodsForCurrentBookingCarAfterFirstSelect[i].car_periods;

        let slicePeriods = periods.filter(
          //оставляем только периоды, которые заканчиваются после начала сравнимого дня
          (period) =>
            this.firstDateOfRange &&
            isAfter(new Date(period.end), this.firstDateOfRange)
        );

        slicePeriods = slicePeriods.filter(
          //оставляем только те периоды, которые начинаются в течении сравнимого дня
          (period) =>
            this.firstDateOfRange &&
            isBefore(
              new Date(period.begin),
              new Date(this.firstDateOfRange).setDate(
                this.firstDateOfRange.getDate() + 1
              )
            )
        );

        const finded = isWithinIntervalsAndFindIt(
          slicePeriods,
          splitDateByMinutes(this.firstDateOfRange, 15)
        );
        if (finded) {
      ...
      findedPeriod.forEach((period: SinglePeriod) => {
        if (isAfter(new Date(period.end), lastEndOfLatestInterval))
          lastEndOfLatestInterval = new Date(period.end);
      });
    ...
    this.freePeriodsForCurrentBookingCarAfterFirstSelect.forEach(
      (item, inx) => {
        ...
        let slicePeriods = item.car_periods.filter(
          //оставляем только периоды, которые заканчиваются после начала сравнимого дня
        ...
        slicePeriods = slicePeriods.filter(
          //оставляем только те периоды, которые начинаются в течении сравнимого дня
          (period) =>
            isBefore(new Date(period.begin), splitDate[splitDate.length - 1])
        );
    ...
    // console.log(this.freePeriodsForCurrentBookingCarAfterFirstSelect);
  ...
  public getFreeTimeSlotsForReceiveAndReturnCar(dt: Date): Date[] {
    ...
    for (
      let j = 0;
      j < this.freePeriodsForCurrentBookingCarAfterFirstSelect.length;
      ++j
    ) {
      let slicePeriods = this.freePeriodsForCurrentBookingCarAfterFirstSelect[
        j
      ].car_periods.filter(
        //оставляем только периоды, которые заканчиваются после начала сравнимого дня
        (period) => isAfter(new Date(period.end), splitDate[0])
      ...
      for (let i = 0; i < splitDate.length; ++i) {
        ...
        if (this.findFirstPeriodWhichConsistTimestamt(slicePeriods, date)) {
          ...
          continuesDurationOfFreePeriods < numberTimeSlotsInFourHours
        ) {
    ...
    if (fourHoursContinuesDurationFounded) {
      ...
      for (let i = 0; i < filledArrayOfFreeTimeSlots.length; ++i) {
        const findItInFreeTimeSlotsArr: boolean =
          freeTimeSlotsForReceiveAndReturnCar.indexOf(
            filledArrayOfFreeTimeSlots[i]
          ) >= 0
            ? true
            : false;
