...
export async function gridRenderer(state: State): Promise<string> {
  ...
  function dayHTML(timestamp: Date) {
    ...
    var isSelected =
      (state.isFirstDateOfRangeWasSelect() &&
        isEqual(state.getFirstDateOfRange(), timestamp)) ||
      (state.isSecondDateOfRangeWasSelect() &&
        isEqual(state.getSecondDateOfRange(), timestamp)) ||
      (state.isFirstDateOfRangeWasSelect() &&
        state.isSecondDateOfRangeWasSelect() &&
  ...
  startDate.setFullYear(state.getSelectedYear());
